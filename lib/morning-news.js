...
/** @returns {{ intro: string, image: Buffer|string }} */
export async function buildMorningNewsParts(intro = DEFAULT_INTRO) {
  ...
  return { intro, image: img };
}

export async function buildMorningNewsReply(intro = DEFAULT_INTRO) {
  const { intro: text, image } = await buildMorningNewsParts(intro);
  return [text, segment.image(image)];
}
...
export async function pushMorningNewsToGroups(groups, delayMs = 0, intro = DEFAULT_INTRO) {
  ...
  let parts;
  try {
    parts = await buildMorningNewsParts(intro);
  } catch (err) {
  ...
  for (const groupId of groups) {
    try {
      ...
      await group.sendMsg(parts.intro);
      await group.sendMsg(segment.image(parts.image));
