...
export const DEPRECATED_HELP_KEYS = [
  'theme', 'bgBlur', 'contBgBlur', 'colWidth', 'bgTheme', 'bgbg', 'bgLayer', 'injectBackground', 'themeExclude'
];
...
export const DEFAULT_HELP_CFG = {
  ...
  style: {
    titleColor: '#f8fafc',
    subTitleColor: 'rgba(248,250,252,0.95)',
    groupColor: '#0f172a',
    accentColor: '#5eead4',
    fontColor: '#0f172a',
    descColor: '#334155',
    footerColor: '#f8fafc',
    contBgColor: 'rgba(255, 255, 255, 0.30)',
    groupBgColor: 'linear-gradient(90deg, rgba(94, 234, 212, 0.28), rgba(255, 255, 255, 0.38))',
    rowBgColor1: 'rgba(248, 250, 252, 0.40)',
    rowBgColor2: 'rgba(241, 245, 249, 0.32)'
  }
};
...
function pickHelpStyle(style) {
  ...
  const panelKeys = new Set(['contBgColor', 'groupBgColor', 'rowBgColor1', 'rowBgColor2']);
  for (const [k, v] of Object.entries(style)) {
    if (DEPRECATED_HELP_STYLE_KEYS.includes(k)) continue;
    if (panelKeys.has(k) && !isUsablePanelAlpha(v)) continue;
    out[k] = v;
  }
  return out;
}
