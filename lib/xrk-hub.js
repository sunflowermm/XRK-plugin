...
class XRKHub {
  ...
  getScreenshotRuntime() {
    ...
    const viewportWidth = cfg.viewport?.width ?? 1536;
    const viewportHeight = cfg.viewport?.height ?? 2138;
    const scale = cfg.quality ?? main.screen_shot_quality ?? cfg.screenshotConfig?.deviceScaleFactor ?? 2;
    return {
      ...
      screenshotConfig: {
        width: cfg.screenshotConfig?.width ?? viewportWidth,
        height: cfg.screenshotConfig?.height ?? viewportHeight,
        deviceScaleFactor: scale,
        waitUntil: cfg.waitUntil ?? cfg.screenshotConfig?.waitUntil ?? 'networkidle2',
        imgType: cfg.screenshotConfig?.imgType ?? 'jpeg',
        fullPage: cfg.screenshotConfig?.fullPage ?? true,
        quality: cfg.screenshotConfig?.quality ?? 100,
