...
import { writeConfigDisk } from './config-persist.js';

const ROOT = process.cwd();
const XRK_PLUGIN_ROOT = path.join(ROOT, 'plugins', 'XRK-plugin');
...
class XRKHub {
  ...
  reload(name) {
    ...
    if (name === 'help_system' && raw && isLegacyHelpRaw(raw)) {
      writeConfigDisk('help_system', meta.ext, sanitizeHelpSystemForDisk(raw));
      logger.info('[XRK-Hub] help_system 已从旧版深色主题迁移为新样式');
    }

    if (name === 'config' && raw == null) {
      parsed = normalizeMainConfig(null);
      writeConfigDisk('config', 'yaml', parsed);
    }
  ...
  set config(value) {
    this._cache.set('config', normalizeMainConfig(value && typeof value === 'object' ? value : null));
  }
  ...
  save() {
    try {
      writeConfigDisk('config', 'yaml', this.config);
