...
export class WebScreenshotUrlService {
  ...
  async prefetchUrl(url) {
    ...
    const opts = { redirect: 'follow', signal: controller.signal, headers: FETCH_HEADERS };
    try {
      ...
      if ([403, 405, 429, 503].includes(res.status)) return true;
      return false;
    } catch (error) {
      try {
        const urlObj = new URL(url);
        if (this.isLocalAddress(urlObj.hostname)) return true;
      } catch { /* ignore */ }
      logger.debug(`[网页截图] URL 预检失败，仍尝试渲染: ${url} (${error.message})`);
      return true;
    } finally {
      clearTimeout(timer);
    }
  ...
  async filterReachableUrls(urls) {
    const valid = [];
    for (const url of urls) {
      if (await this.prefetchUrl(url)) valid.push(url);
    }
    return valid;
  }
}
