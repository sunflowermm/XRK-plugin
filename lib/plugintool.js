...
const HTML_TEMP_DIR = path.join(PLUGINS_DIR, '_tmp');

const PLUGIN_LIST_CSS = `
...
.container {
  width: 960px; max-width: 960px; margin: 0 auto; padding: 28px 32px 24px;
  background: linear-gradient(180deg, rgba(15,23,42,0.55) 0%, rgba(15,23,42,0.82) 100%);
  border-radius: 16px;
}
.header h2 {
  margin: 0 0 18px; text-align: center; font-size: 26px; font-weight: 700;
  color: #f8fafc; letter-spacing: 0.02em; text-shadow: 0 2px 8px rgba(0,0,0,0.35);
}
.plugin-item, .item, li, .row {
  margin: 0 0 10px; padding: 12px 14px; border-radius: 10px;
  background: rgba(255,255,255,0.92); border: 1px solid rgba(255,255,255,0.2);
  box-shadow: 0 2px 8px rgba(15,23,42,0.08); line-height: 1.45; font-size: 14px;
}
.plugin-item h3 { margin: 0 0 8px; font-size: 16px; color: #0f172a; }
.plugin-item p { margin: 4px 0; color: #334155; }
.plugin-item a { color: #0ea5e9; word-break: break-all; }
.footer {
...
export function createHtmlTemplate(title, content) {
  const tpl = FileUtils.readFileSync(PLUGIN_HTML_TEMPLATE, "utf8")
    .replaceAll("{{title}}", title)
    .replace("{{content}}", content);
  // 内联样式：_tmp 下相对 style.css 加载不到
  if (tpl.includes('href="style.css"')) {
    return tpl.replace(
      '<link rel="stylesheet" href="style.css">',
      `<style>${PLUGIN_LIST_CSS}</style>`
    );
  }
  return tpl.replace('</head>', `<style>${PLUGIN_LIST_CSS}</style></head>`);
}

/** 渲染 HTML → 截 .container（勿 fullPage 从顶裁） */
async function renderPluginListScreenshot(htmlContent, saveId, options = {}) {
  FileUtils.ensureDirSync(HTML_TEMP_DIR);
  const htmlPath = path.join(HTML_TEMP_DIR, `${saveId}.html`);
  FileUtils.writeFileSync(htmlPath, htmlContent, "utf8");
  try {
    return await takeScreenshot(htmlPath, `${saveId}_screenshot`, {
      fullPage: false,
      width: 1024,
      deviceScaleFactor: 2,
      imgType: 'png',
      ...
      imageWaitTimeout: 1500,
      ...options
    });
  } finally {
    try {
      await FileUtils.unlink(htmlPath);
    } catch (err) {
      Bot.makeLog?.('debug', `[XRK plugintool] 清理临时 HTML 失败: ${err?.message || err}`, 'XRK-plugin');
    }
  }
}
...
export async function saveAndScreenshot(htmlContent, fileName, options = {}) {
  ...
  FileUtils.ensureDirSync(PLUGINS_DIR);
  const ext = path.extname(fileName) || ".png";
  const base = path.basename(fileName, ext);
  const outPath = path.join(PLUGINS_DIR, `${base}.png`);
  FileUtils.writeFileSync(outPath, buf);
  return outPath;
}
...
function readScreenshotIndex() {
  if (!FileUtils.existsSync(PLUGIN_SCREENSHOT_INDEX)) return {};
  try {
    return JSON.parse(FileUtils.readFileSync(PLUGIN_SCREENSHOT_INDEX, "utf8"));
  } catch {
    return {};
  }
}
...
function writeScreenshotIndex(index) {
  FileUtils.ensureDirSync(PLUGINS_DIR);
  FileUtils.writeFileSync(PLUGIN_SCREENSHOT_INDEX, JSON.stringify(index, null, 2));
}
...
async function loadOrGenerateCategory(category) {
  ...
  if (entry?.hash === currentHash && Array.isArray(entry.files) && entry.files.length > 0) {
    const segments = entry.files
      ...
      .filter(p => typeof p === "string" && FileUtils.existsSync(p))
...
export function execCommand(command, options = {}) {
  if (typeof command !== 'string' || !command.trim()) {
    return Promise.reject(new TypeError(`[插件安装器] 命令必须是字符串，收到: ${typeof command}`));
  }
...
export function resolveMultiplePluginIdentifiers(inputs, pluginMap) {
  ...
  for (const token of inputs) {
    ...
    if (!foundPluginInfo) {
      ...
      const pluginDirs = (FileUtils.readDirSync(pluginsRoot) || [])
        .filter(p => FileUtils.statSync(path.join(pluginsRoot, p))?.isDirectory());
      ...
      if (pluginDirs.includes(token)) {
        ...
        if (FileUtils.existsSync(jsPath) || FileUtils.existsSync(jsPath + '.disable')) {
          foundPluginInfo = { 
            name: token,
...
export function getInstalledPlugins() {
  ...
  const pluginDirs = (FileUtils.readDirSync(pluginDirPath) || [])
    .filter(d => !excluded.includes(d) && FileUtils.statSync(path.join(pluginDirPath, d))?.isDirectory());
  const jsFiles = (FileUtils.readDirSync(exampleDirPath) || [])
    .filter(f => f.endsWith('.js') || f.endsWith('.js.disable'));
  ...
  pluginDirs.forEach(name => {
    const status = (FileUtils.readDirSync(path.join(pluginDirPath, name)) || []).some(f => f.endsWith('.js')) ? '启用' : '已停用';
...
async function generatePluginImages(category, plugins) {
  ...
  for (let [index, group] of groups.entries()) {
    ...
    const outPath = await saveAndScreenshot(htmlContent, baseName);
