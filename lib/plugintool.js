...
import { toImagePayload } from '../lib/web-screenshot.js';
...
export function createHtmlTemplate(title, content) {
  return FileUtils.readFileSync(PLUGIN_HTML_TEMPLATE, "utf8").replaceAll("{{title}}", title).replace("{{content}}", content);
}
...
async function renderPluginListScreenshot(htmlContent, saveId, options = {}) {
  ...
  try {
    ...
    return toImagePayload(raw);
  } finally {
    try { await FileUtils.unlink(htmlPath); } catch {}
  }
}
