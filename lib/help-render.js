...
import { DEFAULT_HELP_CFG, HELP_STYLE_DEFAULTS } from './config-normalize.js';
import hub from './xrk-hub.js';
import { SUB_HELP_PAGES } from './sub-help-pages.js';
import { FileUtils } from '../../../lib/utils/file-utils.js';

export const HELP_PAGE_WIDTH = 1280;
export const HELP_DEVICE_SCALE = 3;
export const HELP_COLUMN_COUNT = DEFAULT_HELP_CFG.columnCount;
...
/** 与 README 预览、子帮助 slug 一一对应，按序分配 bgother 背景（互不重复） */
...
export function resolveBackgroundFile(helpDir, slug = 'help') {
  ...
  const idx = orderIdx >= 0 ? orderIdx % images.length : 0;
  return images[idx];
}
...
export function buildHelpStyleOverrides(helpCfg = {}, bgFileUrl = null) {
  ...
  const bodyBg = bgFileUrl
    ? `background-color:#0f172a;background-image:url('${bgFileUrl}');background-repeat:no-repeat;background-position:center top;background-size:cover;`
...
.cont-box { background: ${style.contBgColor || d.contBgColor}; }
...
export function buildHelpSections(helpList, helpDir = resolveHelpDir(), colCount = HELP_COLUMN_COUNT) {
